	...
	int encode_int(W &writer, const T& v, const Field *field)
	{
		...
		if (field->sub_type == Field::SubNone)
			...
			if (it == meta->enum_values.end())
				return encode_number(writer, v);
			writer.RawValue(it->second.data(), it->second.size(), rapidjson::kStringType);
			return 0;
		}
		return encode_number(writer, v);
	}
...
inline int JSON::encode_field(W &writer, const memoryview<Buf> &data, const Field *field)
{
	...
	case Field::Message:
		...
		return 0;
	case Field::Array: {
		auto cf = field->count_ptr;
		long long size = 0;
		if (cf->type == Field::Int8) size = *data.template dataT<int8_t>();
		else if (cf->type == Field::Int16) size = *data.template dataT<int16_t>();
		else if (cf->type == Field::Int32) size = *data.template dataT<int32_t>();
		else if (cf->type == Field::Int64) size = *data.template dataT<int64_t>();
		else
			return _log.fail(EINVAL, "Invalid count field {} type: {}", cf->name, cf->type);
		if (size < 0)
